...
| CE0854 | Association 'M.A' is not reachable from entity 'M.E' | The association is written (in `create`/`change`) on the non-owner end — write it on the FROM entity (`change $Owner (add $obj to M.A)`), or declare it `owner Both`. `check` flags MDL-ASSOC01 |
